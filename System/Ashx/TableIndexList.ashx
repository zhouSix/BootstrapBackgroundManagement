    ...
    private void GetIndexListJson(HttpContext context)
    {
        ...
        try
        {
            if (!cid.Equals("-1"))
            {
                ...
                string strSql = "select id,title,class_id,iif(class_id=0,'首页',(select sort_name from news_class where id=class_id)) as sort_name,dateandtime,sortid from news where class_id=" + cid + " order by isindex desc, id " + orders;
            ...
            {
                string strSql = "select id,title,class_id,iif(class_id=0,'首页',(select sort_name from news_class where id=class_id)) as sort_name,dateandtime,sortid from news where class_id=-1 order by isindex desc, id " + orders; ;
